...
class MetricsStats():
    ...
    def logstatsline(self, geo, rdot):
        """ returns line to log """
        mask = np.ones((geo.size, geo.size), dtype=bool)
        mask = np.fill_diagonal(mask, 0) # exclude diagonal from data

        strvel = self._partlogline(np.squeeze(self.sim.traf.gs))
        strdvel = self._partlogline(np.sqrt(geo.dVsqr[mask]).flatten())
        strdrange = self._partlogline(geo.qdrdist[:, :, 1][mask].flatten())
        strdhdg = self._partlogline(geo.dhdg[mask].flatten())
